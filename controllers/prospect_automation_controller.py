...
from services.notion_manager import (
    NotionDataManager,
    CampaignProgress,
    CampaignStatus
)
...
from services.domain_validator import (
    extract_valid_domain,
    is_valid_domain
)
...
class ProspectAutomationController:
    ...
    def validate_api_connections(self) -> Dict[str, Any]:
        """
        Validate all API connections and return results.
        ...
        try:
            ...
            results['notion'] = {'status': 'success', 'message': 'Connection successful'}
        except Exception as e:
            results['notion'] = {'status': 'error', 'message': str(e)}
        
        # Test Hunter API
        try:
            self.email_finder.test_connection()
            results['hunter'] = {'status': 'success', 'message': 'Connection successful'}
        except Exception as e:
            results['hunter'] = {'status': 'error', 'message': str(e)}
        ...
        try:
            if hasattr(self.config, 'resend_api_key') and self.config.resend_api_key and self.email_sender:
                ...
                results['resend'] = {'status': 'skipped', 'message': 'Resend API key not configured'}
        except Exception as e:
            ...
            for prospect in target_prospects:
                try:
                    ...
                    try:
                        ...
                        # Explicitly update email generation status to ensure it's set
                        ...
                        try:
                            ...
                            pass  # Don't fail the whole process if status update fails
    ...
    def _discover_companies(self, limit: int) -> List[CompanyData]:
        ...
        try:
            ...
            # Start with a higher fetch limit to account for duplicates
            fetch_limit = max(limit * 3, 10)  # Fetch at least 10 products to find unprocessed ones
            max_attempts = 3
            attempt = 0
            ...
            while len(unprocessed_companies) < limit and attempt < max_attempts:
                ...
                for product in products:
                    try:
                        ...
                        # Allow companies without perfect domains, but log the issue
                        if not domain:
                            self.logger.warning(f"No valid domain found for {product.company_name}, will use fallback methods")
                            # Use a fallback domain based on company name for email finding
                            domain = product.company_name.lower().replace(' ', '').replace('.', '') + '.com'
        ...
        try:
            ...
            try:
                ...
                if stats['emails_sent'] == 0:
                    ...
                    for prospect in prospects:
                        # Check if prospect has email delivery status "Sent" and was sent today
                        if (hasattr(prospect, 'email_delivery_status') and 
                            prospect.email_delivery_status in ["Sent", "Delivered", "Opened", "Clicked"] and
                            hasattr(prospect, 'email_sent_date') and prospect.email_sent_date):
                            try:
                                sent_date = datetime.fromisoformat(prospect.email_sent_date.replace('Z', '+00:00')).date()
                                if sent_date == today:
                                    emails_sent_today += 1
