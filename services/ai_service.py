...
class AIService(BaseService):
    ...
    def _prepare_personalization_data(
        ...
        if sender_profile:
            ...
            data['sender_primary_intro'] = sender_profile.experience_summary[:150] if sender_profile.experience_summary else ''
            data['sender_key_achievement'] = sender_profile.notable_achievements[0] if sender_profile.notable_achievements else ''
            data['sender_value_connection'] = sender_profile.value_proposition[:100] if sender_profile.value_proposition else ''
            data['sender_availability_note'] = sender_profile.availability or 'Available for new opportunities'
            
            # Create dynamic sections for natural integration
            data['sender_intro_section'] = f"I'm {sender_profile.name}, {sender_profile.current_role or 'a professional'} with {sender_profile.years_experience or 'several'} years of experience"
            data['skill_connection_section'] = f"My background in {', '.join(sender_profile.key_skills[:3])} aligns well with your needs"
            data['achievement_section'] = f"Recently, {sender_profile.notable_achievements[0]}" if sender_profile.notable_achievements else ''
            data['value_prop_section'] = sender_profile.value_proposition[:200] if sender_profile.value_proposition else ''
            data['availability_section'] = f"I'm {sender_profile.availability or 'currently available'} and {sender_profile.remote_preference or 'open to various arrangements'}"
            data['portfolio_section'] = f"You can see my work at {sender_profile.portfolio_links[0]}" if sender_profile.portfolio_links else ''
    ...
    def _parse_generated_email_content(self, generated_content: str) -> tuple[str, str]:
        ...
        # Look for subject line in various formats
        for i, line in enumerate(lines):
            ...
            if line_lower.startswith('subject:'):
                ...
                subject = line[12:].strip()  # Remove "**Subject:**" prefix
                body = '\n'.join(lines[i+1:]).strip()
                break
            elif '**subject:**' in line_lower:
                # Handle format like "**Subject:** text"
                subject_start = line_lower.find('**subject:**') + 12
                subject = line[subject_start:].strip()
                body = '\n'.join(lines[i+1:]).strip()
                break
...
The email should:
1. Open with how I found them on ProductHunt and genuine interest in their product/mission
2. Introduce me with the most relevant aspects of my background for their specific needs
3. Connect my experience/skills to their company's challenges or growth stage
4. Reference specific product features or business insights to show genuine understanding
5. Present my value proposition in context of what they're building
6. Include a natural mention of my availability and location preferences
7. End with a soft call-to-action for a brief conversation

Write the actual email now - no meta-commentary, no placeholders, just the complete email."""
